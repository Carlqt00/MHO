...
- **SMS:** iTextMo (one-SIM Android gateway, `api.itextmo.com/v1/messages`) via Edge Functions — see `supabase/functions/README.md`. Email: not implemented (no provider chosen).
...
5. Notifications — SMS confirmations + lifecycle updates (booked, rescheduled, cancelled, checked in, now serving, served, no-show), password-reset codes, announcement broadcasts. No 24h reminders (no scheduler/cron in this stack). Email not implemented.
...
- [x] **Step 4:** Live queue board (staff side, realtime) — QueueBoard component on /staff and /nurse, grouped per provider, "Done, Call Next" via atomic advance_queue RPC (advisory lock). Realtime = signal-only refetch on queue_tickets changes. `0007_queue_board.sql` **applied & verified 2026-08-21** (nurse read policies + RPC + realtime publication). Realtime sync across devices pending user verification.
...
- [~] **Step 6:** Announcements + notifications — **announcements DONE**, notifications remaining.
  ...
  - ✅ **Notifications (DONE 2026-09-19, real iTextMo SMS):** every send goes through `_shared/sms.ts` → `notification_logs` (`0020` + `0021`: `event`, `provider_message_id`, `announcement_id`, status `delivered`). Surfaces: (a) appointment lifecycle via `send-sms` — booked / cancelled / **rescheduled** (new `reschedule_appointment` RPC, patient `?reschedule=<id>` wizard mode + admin picker on Providers → exception conflicts) / **checked in** + **no-show** (new `set_appointment_status` RPC, buttons on the Queue board) / **now serving** + **served** (from `advance_queue`, which now returns both appointment ids); (b) **password reset** = 6-digit SMS code (`password-reset-request` texts it, `password-reset-complete` verifies with a 5-attempt cap; the code never reaches the browser); (c) **announcements** = explicit admin "Send via SMS" with recipient count (`send-announcement-sms`, resumable); (d) admin free-text send; (e) `itextmo-webhook` verifies the HMAC signature and reconciles sent/delivered/failed. The legacy `notifications` table (`0001`) is unused. **No 24h reminders** — deliberately skipped, there is no scheduler. Email not implemented.
...
| 2026-08-21 | Keep migration-file comment blocks SHORT — long lines mangle when pasted through a terminal | The `0016` header comments wrapped/broke when copied through the terminal to hand-apply the migration. Migrations here are applied by paste, so keep comment lines to roughly one short clause (well under ~80 chars) and prefer several short lines over one long wrapped sentence, so the SQL survives the paste intact. The SQL statements themselves are already short; this is specifically about the leading `--` comment block. |
| 2026-09-19 | SMS provider is **iTextMo**; every text is composed server-side and logged before it is sent | iTextMo is a one-SIM handset gateway (1 msg/sec, 1000 queue, 2000/day, per-recipient 5/10 min). The client never supplies SMS text for lifecycle events — `send-sms` builds it from the appointment row and refuses events that don't match the row's real status, so a caller can only trigger the notification that is actually true. Each attempt is a `notification_logs` row created *before* the gateway call (status flips to sent/failed; the webhook later flips sent→delivered), so a crash mid-send still leaves evidence. The gateway forgets history after 7 days; our log is the record. |
| 2026-09-19 | Password reset uses a 6-digit **SMS code**, hashed with the request id, 5 attempts, 15 min | The previous flow handed the reset token straight back to the browser after a name/email/phone match — knowledge only, no possession. Texting a code makes the registered phone the second factor. The code is short, so it is hashed as `sha256(requestId:code)` (a leaked row can't be brute-forced offline without the id) and `code_attempts` voids the request after 5 misses. If the SMS fails the request is marked `failed` immediately so the 60 s resend cooldown never traps the user. |
| 2026-09-19 | Announcement SMS is an **explicit admin button with a recipient count**, never a side effect of publish | Publishing is cheap and reversible; a broadcast to every patient burns the SIM's daily quota and cannot be unsent. So the admin sees "Text this to N patients?" and confirms. The function skips patients already sent that announcement, so a partial run (time budget, QUEUE_FULL) is finished by clicking again rather than by re-texting everyone. |
| 2026-09-19 | No 24-hour appointment reminders | There is no scheduler in this stack (no pg_cron / no cron job) and the user chose not to add one. Reminders would need either pg_cron + pg_net calling `send-sms` with a service token, or an external cron hitting a new function. Left out on purpose; everything else in the notification plan is event-driven and implemented. |
